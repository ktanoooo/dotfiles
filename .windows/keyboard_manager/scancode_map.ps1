$hexified = "00,00,00,00,00,00,00,00,07,00,00,00,1d,e0,3a,00,5b,e0,1d,00,38,00,5b,e0,1d,00,38,00,29,00,7b,00,29,00,79,00,00,00,00,00".Split(',') | % { "0x$_"};
$kbLayout = 'HKLM:\System\CurrentControlSet\Control\Keyboard Layout';
New-ItemProperty -Path $kbLayout -Name "Scancode Map" -PropertyType Binary -Value ([byte[]]$hexified) -Force;
...
# 1d,e0,3a,00  // CAPS LOCK -> RightCtrl
...
#
# CAPS LOCK は Mac の Control と同じ位置にあるため、Ctrl として扱う。
# 左右を分けるのが要点で、コピー & ペーストに使う左 Ctrl(物理 Alt)とは別物になる。
# AutoHotkey 側は右 Ctrl だけを捕まえるので、左 Ctrl の挙動は一切変わらない。
